...
## Floor a CoinPaprika year-history window at 364 days, not 365

- **Context**: Any code fetching ~1 year of CoinPaprika daily history and deriving the window start — `src/lib/coinpaprika.ts` `getHistoricalPriceSeries`, and window-start logic like `portfolio-history-service.ts` `WINDOW_FLOOR_DAYS`.
- **Problem**: CoinPaprika's free tier rejects a `start` at exactly 365 days ago — its cutoff is a precise *rolling* timestamp just under 365 days (e.g. "2025-06-26 22:47 UTC"), so a midnight start one full year back returns HTTP 402. `safeFetch` degrades that to an empty series, so every asset silently drops to 0 (a visible cliff to $0). Found live during portfolio-history-chart Phase 1 manual verification.
- **Rule**: When fetching ~1 year of CoinPaprika daily history, floor the window start at ~364 days ago, never exactly 365. Treat the free-tier historical cutoff as a rolling sub-365-day timestamp, not a clean date boundary.
- **Applies to**: plan, implement

## Verify price-provider reachability from the deployed Worker, not localhost

- **Context**: Any change to the price boundary (`src/lib/prices.ts`) or its provider. Surfaced when Binance — which worked locally and passed the binance-price-provider p4 "reachable from Worker" check — began returning HTTP 403 to the Cloudflare Worker's egress IP days later, and its replacement Coinbase turned out firewall-blocked from the dev network but reachable from the Worker (the inverse).
- **Problem**: `fetch` egresses from a different IP depending on where it runs — localhost/dev uses the machine or corporate IP; the deployed Worker uses Cloudflare's datacenter IP. Providers treat these differently: Binance 403s Cloudflare datacenter IPs, while the dev corporate network firewalls Coinbase outright (HTTP 000). So a provider that works on `npm run dev` can be dead in production and vice-versa. Both failures degrade silently to `null`, reading as "price unavailable" with no cause — exactly how the CoinPaprika 402 and Binance 403 outages stayed invisible.
- **Rule**: Verify price-provider reachability from the **deployed Worker** (e.g. a temporary endpoint that runs the upstream fetch server-side and returns the raw status), never from localhost or a local curl. When a block is suspected, check the actual upstream status — the boundary's degrade-to-null hides it. Keep the `safeFetch` failure `console.warn` so the next block shows up in `wrangler tail` instead of silently.
- **Applies to**: plan, implement
