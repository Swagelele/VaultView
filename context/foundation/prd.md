...
- **When** they select a transaction type (BUY/SELL/SWAP), pick the source and target assets (with autocomplete from the pricing API's asset list), enter quantities, accept or override the API-suggested price, set a fee, pick date/time and location, and submit
...
- FR-005: User can add a DEPOSIT (one-sided: adds asset quantity at a location). User specifies the original purchase date; the app derives cost basis from the pricing API's historical price at that date, enabling P&L calculation for deposited assets. Priority: must-have
  > Socrates: Counter-argument considered: "depositing without cost basis makes unrealized P&L impossible to calculate." Resolution: DEPOSIT now requires the original purchase date. App derives cost basis from historical API price, so P&L is calculable for all assets including deposits.
...
- FR-007: User sees a price suggested by the external pricing API (current or historical for the selected date/time) when adding a transaction, and can override it manually. Priority: must-have
...
- A user sees acknowledgement of any form submission within 1 second, and price suggestions load within 3 seconds of selecting an asset and date/time.
...
VaultView calculates realized and unrealized P&L using the Average Cost method across five crypto transaction types (BUY, SELL, SWAP as two-sided trades; DEPOSIT with historical cost basis; WITHDRAW as cash-out), consolidating positions by asset and location label, with prices sourced from an external cryptocurrency pricing API.

The rule consumes: transaction records (type, source/target assets, quantities, exchange rate, fee, date/time, location) and historical/current prices from the external pricing API. Location labels are user-defined free-text tags created inline during transaction entry.
...
The user encounters the output every time they view the portfolio — the P&L engine recalculates from the full transaction history. Every trade (BUY/SELL/SWAP) is a two-sided crypto-to-crypto exchange: the source asset's average cost determines realized P&L, and the exchange price becomes the target asset's cost basis for future unrealized P&L. DEPOSIT derives cost basis from the pricing API's price at the user-specified original purchase date. WITHDRAW realizes P&L at current market price and removes the asset from tracking permanently.
...
- No automatic import from exchanges — all transactions entered manually. API import from exchanges and wallets is out of scope for MVP.
...
1. **WITHDRAW pricing mechanism** — When a user withdraws an asset, P&L is realized. At what price? Current market price at time of withdrawal? Or user-specified? Owner: user. Block: no (default to current market price, refine if needed).
