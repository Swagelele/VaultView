...
export function PortfolioTable({ assets, showClosed, onSellAllSuccess }: PortfolioTableProps) {
  ...
  const [sellAllAsset, setSellAllAsset] = useState<PortfolioAsset | null>(null);
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-white/10">
            ...
            <TableHead className="w-20 text-white/70"></TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {visible.map((a) => (
            <Fragment key={a.asset}>
              ...
              >
                <TableCell className="w-8 pr-0">
                  <ChevronRightIcon
                    className={cn("size-4 transition-transform", expanded.has(a.asset) && "rotate-90")}
                  />
                </TableCell>
                ...
                <TableCell className="text-right">
                  <button
                    type="button"
                    className={cn(
                      "cursor-pointer text-xs font-medium text-red-400 hover:text-red-300 hover:underline",
                      (a.is_closed || a.total_quantity <= 0) && "cursor-not-allowed opacity-50",
                    )}
                    disabled={a.is_closed || a.total_quantity <= 0}
                    onClick={(e) => {
                      e.stopPropagation();
                      setSellAllAsset(a);
                    }}
